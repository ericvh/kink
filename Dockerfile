...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -a -tags netgo -ldflags '-w -extldflags "-static"' -o bin/kink main.go
...
ARG ARCH=arm64 #arm64 # arm64 is the default, not needed
ARG K3S_URL=

RUN K3S_FILENAME="k3s-arm64" \
...
RUN mkdir /tmp/etcd-download-test \
 && curl -fvL ${ETCD_URL}/${ETCD_VERSION}/etcd-${ETCD_VERSION}-linux-arm64.tar.gz \
  | tar xzv -C /tmp/etcd-download-test --strip-components=1 \
 && mv /tmp/etcd-download-test/etcdctl /usr/local/bin/

ARG YQ_VERSION=v4.28.2
RUN curl -fvL "https://github.com/mikefarah/yq/releases/download/${YQ_VERSION}/yq_linux_arm64" > /usr/local/bin/yq \
...
RUN curl -fvL https://get.helm.sh/helm-${HELM_VERSION}-linux-arm64.tar.gz \
